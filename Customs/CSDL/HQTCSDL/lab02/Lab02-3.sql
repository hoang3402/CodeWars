...
go
create function XepLoai (@Diem as float) 
returns nvarchar(4) 
as 
    begin
    return case
...
go
create function TachHoTen (@HoTen as nvarchar(50)) returns nvarchar(50) begin
    declare @RHoTen nvarchar(50)
    set
    @RHoTen = reverse(@HoTen)
    return reverse((select substring(@RHoTen, 1, charindex(' ', @RHoTen) - 1)))
end
go
select
    dbo.TachHoTen('Nguyen Van A')
select
    dbo.TachHoTen('Trần Văn Hoàng')
select
    dbo.TachHoTen('Lê Thị Sen')
select
    dbo.TachHoTen('Nguyễn Văn Huy')
select
    dbo.TachHoTen('Nguyễn Thị Cẩm Tú')
go
...
go
create function DocDiem (@Diem as int) returns nvarchar(20) as begin
...
begin
    ...
    from
        DiemHP
        ,HOCPHAN
    where   (DiemHP.MaHP = HOCPHAN.MaHP)
...
go
create function TongDVHT (@MaSV as int) 
returns int 
begin
    return (
        select
        SUM(SoDVHT) as SoDVHT
    from
        SINHVIEN
        ,DiemHP
        ,HOCPHAN
...
go
create function SoSV() returns int begin
...
go
create function SoHP() returns int begin
    -- Giống SoSV()

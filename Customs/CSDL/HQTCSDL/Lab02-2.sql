...
select
    ...
    ,HoTen
    ,MaLop
    ,format(SINHVIEN.NgaySinh, 'dd/mm/yyyy')
    ,GioiTinh
from
    SINHVIEN
where
...
from
    SINHVIEN
where
    (DATEDIFF(YEAR, NgaySinh, GETDATE()) >= 19)
...
select
    ...
    ,TenLop
    ,GioiTinh
    ,COUNT(GioiTinh) as N'số sinh viên'
from
    SINHVIEN
    ,LOP
where
...
group by
    ...
    HoTen
having
    ROUND(SUM(DiemHP * SoDVHT) / SUM(SoDVHT), 1) < 3
...
group by
    SINHVIEN.MaSV, HoTen
having ROUND(SUM(DiemHP * SoDVHT) / SUM(SoDVHT), 1) >= all ( select ROUND(SUM(DiemHP * SoDVHT) / SUM(SoDVHT), 1) as DiemTBC
from
    SINHVIEN ,DIEMHP ,HOCPHAN
where
    (DIEMHP.MaSV = SINHVIEN.MaSV)
...
select
    distinct HOCPHAN.MaHP
    ,HOCPHAN.TenHP
from
    dbo.DIEMHP
    inner join dbo.HOCPHAN on DIEMHP.MaHP = HOCPHAN.MaHP
where
    DIEMHP.DiemHP < 5
group by
    HOCPHAN.MaHP,
...
group by
    ...
    NgaySinh
having
    DATEDIFF(YEAR, NgaySinh, GETDATE()) >= all (
        select
    DATEDIFF(YEAR, NgaySinh, GETDATE())
from
    SINHVIEN
...
from
    dbo.DIEMHP
    inner join dbo.SINHVIEN on DIEMHP.MaSV = SINHVIEN.MaSV
    inner join dbo.HOCPHAN on DIEMHP.MaHP = HOCPHAN.MaHP
where
    DIEMHP.DiemHP >= all (
        select
...
from
    dbo.SINHVIEN
    inner join dbo.DIEMHP on SINHVIEN.MaSV = DIEMHP.MaSV
where
    DIEMHP.DiemHP >= all (
        select
...
group by
            SINHVIEN.MaSV
having
            ROUND(SUM(DIEMHP.DiemHP * HOCPHAN.SoDVHT) / SUM(HOCPHAN.SoDVHT),1) < 3
    )
...
having
            ROUND(
                SUM(DIEMHP.DiemHP * HOCPHAN.SoDVHT) / SUM(HOCPHAN.SoDVHT),
                1
            ) >= 5
    )
update
...
alter table
    dbo.SINHVIEN
add
    XetLenLop NVARCHAR(30)
update
    dbo.SINHVIEN
set
    XetLenLop = N'Buộc thôi học'
where
    MaSV in (
        select
    SINHVIEN.MaSV
...
update
    dbo.SINHVIEN
set
    XetLenLop = N'Tạm ngừng tiến độ học tập'
where
    MaSV in (
        select
    SINHVIEN.MaSV
